...
for i=1:numStacks
    ...
    saveFolder=strcat(name(1:end-5),'NNAnalysis');
    ...
    saveHist(predsFiltered,name,saveFolder,path); %creates movie hist and saves it
...
runHist(runAnglesFiltered) %creates and saves summary histogram
...
fprintf(sumFileID,'%s\n',strcat('numStacks=',num2str(numStacks)));
...
for k=1:numStacks %will split up every stack 
    ...
    folderName=strcat(stackName(1:end-4),'Split'); %foldername based on stack name
...
function out=saveHist(predsFiltered,name,saveFolder,path)
%This function will create and save histograms for each movie
    ...
    dim = [.70 .5 .3 .3];
    str = {strcat('mean:',num2str(nanmean(abs(predsFiltered)))),strcat('std:',num2str(nanstd(abs(predsFiltered))))};
...
dim = [.70 .5 .3 .3];
str = {strcat('mean:',num2str(nanmean(abs(runAnglesFiltered)))),strcat('std:',num2str(nanstd(abs(runAnglesFiltered))))};
annotation('textbox',dim,'String',str,'FitBoxToText','on');
...
function writeNewStacks(name,stack_ds,predsDouble,maxScores,framesFiltered)
    ...
    for k=1:numFrames %loops through frames of stack
        savePath=strcat(pwd,'/NNStacks/',name(1:end-5),'NNAnalysis/',name(1:end-5),'_NNAnglesInserted.tif');
...
function [fitResult, gof] =truncFoldNorm(predsFiltered)

edges=(0:5:180); %for binning
centers=(0:5:177.5)+2.5; %for binning

mba = abs(predsFiltered); %taking abs_value of the end to end bending angle
...
% fitting to truncated folded normal distribution

[xData, yData] = prepareCurveData(centers,counts);

% Set up fittype and options.
ft = fittype( 'c*sqrt(2/(pi*l))*((exp(-(x-m)^2/(2*l^2))+exp(-(x+m)^2/(2*l^2)))/(erf((180+m)/sqrt(2*l^2))+erf((180-m)/sqrt(2*l^2))-erf((m)/sqrt(2*l^2))-erf((-m)/sqrt(2*l^2))))'...
    , 'independent', 'x', 'dependent', 'y' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares','StartPoint' ,start,'Lower' ,lower, 'Upper', upper,...
    'DiffMaxChange', DMaxC, 'TolX', TolX);
opts.Display = 'Off';
